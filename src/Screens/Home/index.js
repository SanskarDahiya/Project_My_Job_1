...
const Home = () => {
  ...
  const [{ data: jobs, loading: jobsLoading, error: jobsError }, executeUserInfo] = useAxios(getJobConfig, {
    manual: true
  });
  ...
  return (
    <div style={{ display: "flex", padding: 50, flexDirection: "column", alignItems: "flex-start" }}>
      <div>
        <HomeFilled style={{ color: "#ffffff", marginRight: 10 }} />
      ...
      <List
        ...
        count={Number(jobs?.metadata?.count / 20).toFixed(0)}
      />
    </div>
  );
};
